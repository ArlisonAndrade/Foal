  ...
  for (const r of registros) {
    try {
      const props = {
        ...
        Momento:   { rich_text: [{ text: { content: r.momento || "—" } }] },
      };
      ...
      if (r.resultados) {
        for (const [attr, dados] of Object.entries(r.resultados)) {
          if (dados.nota !== null) {
            props[`${dados.nome} — Nota`]  = { number: dados.nota };
            props[`${dados.nome} — Menção`] = { rich_text: [{ text: { content: dados.mencao } }] };
          } else {
            props[`${dados.nome} — Menção`] = { rich_text: [{ text: { content: "NO" } }] };
          }
        }
      }
